...
const Room6: React.FC = () => {
  ...
  useEffect(() => {
    if (!question) {
      const availableQuestions = questionBank.filter(q => !state.usedQuestions.includes(q.id) && !q.isWorldCup && q.level === 6);
      if (availableQuestions.length > 0) {
        const randomIndex = Math.floor(Math.random() * availableQuestions.length);
        setQuestion(availableQuestions[randomIndex]);
      } else {
        const fallbackQs = questionBank.filter(q => !q.isWorldCup && q.level === 6);
  ...
  const handleAnswer = (index: number) => {
    ...
    if (isCorrect) {
      setFeedback('correct');
      setTimeout(() => {
        answerQuestion(true, question.id, '/room7');
        navigate('/room7');
      }, 2000);
    } else {
      ...
      setTimeout(() => {
        setFeedback(null);
        answerQuestion(false, question.id, '/room7');
      }, 3000);
    }
  };
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        alignItems: 'center',
        background: 'linear-gradient(rgba(10, 14, 23, 0.8), rgba(10, 14, 23, 0.95)), url("https://images.unsplash.com/photo-1614030635489-0b15a6b0c950?q=80&w=1920") center/cover',
      }}
    >
      <h1 style={{ marginBottom: '20px' }}>Sala 6: El Laboratorio Secreto</h1>
