...
const Room3: React.FC = () => {
  ...
  useEffect(() => {
    if (!question) {
      const availableQuestions = questionBank.filter(q => !state.usedQuestions.includes(q.id) && !q.isWorldCup && q.level === 3);
  ...
  const handleAnswer = (index: number) => {
    ...
    if (isCorrect) {
      setFeedback('correct');
      setTimeout(() => {
        answerQuestion(true, question.id, '/room4');
        navigate('/room4');
      }, 2000);
    } else {
      ...
      setTimeout(() => {
        setFeedback(null);
        answerQuestion(false, question.id, '/room4');
      }, 3000);
    }
  };
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        alignItems: 'center',
        background: 'linear-gradient(rgba(10, 14, 23, 0.8), rgba(10, 14, 23, 0.95)), url("https://images.unsplash.com/photo-1507842217343-583bb7270b66?q=80&w=1920") center/cover',
      }}
    >
      <h1 style={{ marginBottom: '20px' }}>Sala 3: La Biblioteca Oculta</h1>
