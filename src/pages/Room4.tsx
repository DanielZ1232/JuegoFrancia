...
const Room4: React.FC = () => {
  ...
  useEffect(() => {
    if (!question) {
      const availableQuestions = questionBank.filter(q => !state.usedQuestions.includes(q.id) && !q.isWorldCup && q.level === 4);
      if (availableQuestions.length > 0) {
        const randomIndex = Math.floor(Math.random() * availableQuestions.length);
        setQuestion(availableQuestions[randomIndex]);
      } else {
        const fallbackQs = questionBank.filter(q => !q.isWorldCup && q.level === 4);
  ...
  const handleAnswer = (index: number) => {
    ...
    if (isCorrect) {
      setFeedback('correct');
      setTimeout(() => {
        answerQuestion(true, question.id, '/room5');
        navigate('/room5');
      }, 2000);
    } else {
      ...
      setTimeout(() => {
        setFeedback(null);
        answerQuestion(false, question.id, '/room5');
      }, 3000);
    }
  };
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        alignItems: 'center',
        background: 'linear-gradient(rgba(10, 14, 23, 0.8), rgba(10, 14, 23, 0.95)), url("https://images.unsplash.com/photo-1541339907198-e08756dedf3f?q=80&w=1920") center/cover',
      }}
    >
      <h1 style={{ marginBottom: '20px' }}>Sala 4: Cámara del Tesoro</h1>
