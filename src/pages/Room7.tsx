...
const Room7: React.FC = () => {
  ...
  useEffect(() => {
    if (!question) {
      const availableQuestions = questionBank.filter(q => !state.usedQuestions.includes(q.id) && !q.isWorldCup && q.level === 7);
      if (availableQuestions.length > 0) {
        const randomIndex = Math.floor(Math.random() * availableQuestions.length);
        setQuestion(availableQuestions[randomIndex]);
      } else {
        const fallbackQs = questionBank.filter(q => !q.isWorldCup && q.level === 7);
  ...
  const handleAnswer = (index: number) => {
    ...
    if (isCorrect) {
      setFeedback('correct');
      setTimeout(() => {
        answerQuestion(true, question.id, '/final');
        navigate('/final');
      }, 2000);
    } else {
      ...
      setTimeout(() => {
        setFeedback(null);
        answerQuestion(false, question.id, '/final');
      }, 3000);
    }
  };
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        alignItems: 'center',
        background: 'linear-gradient(rgba(10, 14, 23, 0.8), rgba(10, 14, 23, 0.95)), url("https://images.unsplash.com/photo-1549488344-1f9b8d2bd1f3?q=80&w=1920") center/cover',
      }}
    >
      <h1 style={{ marginBottom: '20px' }}>Sala 7: La Cúpula del Museo</h1>
