...
const Room1: React.FC = () => {
  ...
  useEffect(() => {
    if (!question) {
      const availableQuestions = questionBank.filter(q => !state.usedQuestions.includes(q.id) && !q.isWorldCup && q.level === 1);
      if (availableQuestions.length > 0) {
        const randomIndex = Math.floor(Math.random() * availableQuestions.length);
        setQuestion(availableQuestions[randomIndex]);
      } else {
        const fallbackQs = questionBank.filter(q => !q.isWorldCup && q.level === 1);
  ...
  const handleAnswer = (index: number) => {
    ...
    if (isCorrect) {
      setFeedback('correct');
      setTimeout(() => {
        answerQuestion(true, question.id, '/room2');
        navigate('/room2');
      }, 2000);
    } else {
      ...
      setTimeout(() => {
        setFeedback(null);
        answerQuestion(false, question.id, '/room2');
      }, 3000);
    }
  };
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        alignItems: 'center',
        background: 'linear-gradient(rgba(10, 14, 23, 0.8), rgba(10, 14, 23, 0.95)), url("https://images.unsplash.com/photo-1542614539-7517ea5ca528?q=80&w=1920") center/cover',
      }}
    >
      <h1 style={{ marginBottom: '20px' }}>Sala 1: Hall Principal</h1>
